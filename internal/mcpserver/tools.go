...
// registerTools registers all read-only tools on the given MCP server.
func (s *server) registerTools(m *mcp.Server) {
	mcp.AddTool(m, &mcp.Tool{
		Name:        "list_players",
		Description: "List players, optionally filtered by a search string (matches id, display name, or email prefix).",
	}, s.listPlayers)

	mcp.AddTool(m, &mcp.Tool{
		Name:        "get_player",
		Description: "Get a single player by their numeric id.",
	}, s.getPlayer)

	mcp.AddTool(m, &mcp.Tool{
		Name:        "get_player_by_email",
		Description: "Get a single player by their email address.",
	}, s.getPlayerByEmail)

	mcp.AddTool(m, &mcp.Tool{
		Name:        "get_player_stats",
		Description: "Get aggregate stats for a player within an optional date range.",
	}, s.getPlayerStats)

	mcp.AddTool(m, &mcp.Tool{
		Name:        "get_player_profile",
		Description: "Get the full profile for a player (stats, tables, and graph data) within an optional date range.",
	}, s.getPlayerProfile)

	mcp.AddTool(m, &mcp.Tool{
		Name:        "list_player_tables",
		Description: "List the tables a player belongs to, optionally filtered by a date range.",
	}, s.listPlayerTables)
	...
	mcp.AddTool(m, &mcp.Tool{
		Name:        "get_table",
		Description: "Get a single table by its uuid, including the number of games played.",
	}, s.getTable)

	mcp.AddTool(m, &mcp.Tool{
		Name:        "get_table_roster",
		Description: "Get the roster of players at a table, including their table permissions and balances.",
	}, s.getTableRoster)
...
func (s *server) listPlayers(ctx context.Context, _ *mcp.CallToolRequest, in listPlayersInput) (*mcp.CallToolResult, listPlayersOutput, error) {
	offset, limit, err := parsePagination(in.Start, in.Rows)
	if err != nil {
		return nil, listPlayersOutput{}, err
	}
	...
	return nil, listPlayersOutput{Players: fromPlayers(players)}, nil
}
...
func (s *server) getPlayer(ctx context.Context, _ *mcp.CallToolRequest, in getPlayerInput) (*mcp.CallToolResult, PlayerDTO, error) {
	player, err := s.repos.Players.GetPlayerByID(ctx, in.ID)
	if err != nil {
		return nil, PlayerDTO{}, notFound(err, "player")
	}

	return nil, fromPlayer(player), nil
}
...
func (s *server) getPlayerByEmail(ctx context.Context, _ *mcp.CallToolRequest, in getPlayerByEmailInput) (*mcp.CallToolResult, PlayerDTO, error) {
...
func (s *server) getPlayerStats(ctx context.Context, _ *mcp.CallToolRequest, in getPlayerStatsInput) (*mcp.CallToolResult, PlayerStatsDTO, error) {
	from, to, err := parseDateRange(in.From, in.To)
	if err != nil {
		return nil, PlayerStatsDTO{}, err
	}

	stats, err := s.repos.Players.GetPlayerStats(ctx, in.ID, from, to)
	if err != nil {
		return nil, PlayerStatsDTO{}, notFound(err, "player")
	}

	return nil, fromPlayerStats(stats), nil
}
...
func (s *server) getPlayerProfile(ctx context.Context, _ *mcp.CallToolRequest, in getPlayerProfileInput) (*mcp.CallToolResult, PlayerProfileDTO, error) {
	from, to, err := parseDateRange(in.From, in.To)
	if err != nil {
		return nil, PlayerProfileDTO{}, err
	}
	...
	if err != nil {
		return nil, PlayerProfileDTO{}, notFound(err, "player")
	}

	return nil, fromPlayerProfile(profile), nil
}
...
func (s *server) listPlayerTables(ctx context.Context, _ *mcp.CallToolRequest, in listPlayerTablesInput) (*mcp.CallToolResult, listPlayerTablesOutput, error) {
	offset, limit, err := parsePagination(in.Start, in.Rows)
	if err != nil {
		return nil, listPlayerTablesOutput{}, err
	}
	...
	if in.From == nil && in.To == nil {
		player, err := s.repos.Players.GetPlayerByID(ctx, in.ID)
		if err != nil {
			return nil, listPlayerTablesOutput{}, notFound(err, "player")
		}
		...
		return nil, listPlayerTablesOutput{Tables: fromTablesWithBalance(tables)}, nil
	}
...
func (s *server) listTables(ctx context.Context, _ *mcp.CallToolRequest, in listTablesInput) (*mcp.CallToolResult, listTablesOutput, error) {
	offset, limit, err := parsePagination(in.Start, in.Rows)
	if err != nil {
	...
	return nil, listTablesOutput{Tables: fromTablesWithEmail(tables)}, nil
}
...
func (s *server) getTable(ctx context.Context, _ *mcp.CallToolRequest, in getTableInput) (*mcp.CallToolResult, getTableOutput, error) {
	table, err := s.repos.Tables.GetTableByUUID(ctx, in.UUID)
	if err != nil {
		return nil, getTableOutput{}, notFound(err, "table")
	}

	count, err := s.repos.Tables.GetGamesCount(ctx, table)
	if err != nil {
		return nil, getTableOutput{}, err
	}

	return nil, getTableOutput{Table: fromTable(table), GamesCount: count}, nil
}
...
func (s *server) getTableRoster(ctx context.Context, _ *mcp.CallToolRequest, in getTableRosterInput) (*mcp.CallToolResult, getTableRosterOutput, error) {
	table, err := s.repos.Tables.GetTableByUUID(ctx, in.UUID)
	if err != nil {
		return nil, getTableRosterOutput{}, notFound(err, "table")
	}

	players, err := s.repos.Tables.GetPlayers(ctx, table)
	if err != nil {
		return nil, getTableRosterOutput{}, err
	}

	return nil, getTableRosterOutput{Table: fromTable(table), Players: fromPlayerTables(players)}, nil
}
...
func (s *server) listGameTypes(_ context.Context, _ *mcp.CallToolRequest, _ listGameTypesInput) (*mcp.CallToolResult, listGameTypesOutput, error) {
	return nil, listGameTypesOutput{GameTypes: gamefactory.Names()}, nil
}
